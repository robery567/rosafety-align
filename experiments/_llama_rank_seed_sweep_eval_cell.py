"""
Llama rank x seed sweep eval cell.

Evaluates the 4 Llama rank-seed-sweep adapters trained by the corresponding
training cell (Llama x {r=64, r=128} x {seed=1729, seed=65537}) on the
held-out split. Saves per-(anchor, rank, seed) safety.json files.

Estimated cost:
  - Generation: ~25 min total on A100 (4 adapters x ~6 min each).
  - Judging:    ~$1-2 OpenRouter (gpt-5-mini judge; cache hits are high
                because the prompts are identical to previous evals).
"""
...
# --- Llama rank x seed sweep eval config ---
...
def _eval_one(lora_r, seed):
    ...
    if out_path.exists():
        print(f"[Llama r={lora_r} seed{seed}] {out_path.name} already exists; skipping.")
        return
    if not adapter_a.exists():
        print(f"[Llama r={lora_r} seed{seed}] adapter {adapter_a} not found; "
              f"run the Llama rank-seed-sweep training cell in nb03 first.")
        return

    print(f"\n=== Llama rank-seed-sweep eval r={lora_r} seed={seed} -> {out_path.name} ===")
...
for seed in NEW_SEEDS:
    for lora_r in RANKS:
        _eval_one(lora_r, seed)
print("\nLlama rank-seed-sweep batch eval done.")
